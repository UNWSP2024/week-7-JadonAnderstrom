...
def main():
    ...
    # Continuously collect user input until the user enters 0 for the year
    while True:
        year = int(input("Enter the year (or 0 to quit): "))
        if year == 0:
            break
    ...
    total_population = sum_population_for_year(all_entered_values, target_year)
...
def sum_population_for_year(all_entered_values, year_to_sum):
    ...
    # Iterate through the list of lists, summing populations for the target year
    for year, state, population in all_entered_values:
        if year == year_to_sum:
...
# Jadon Anderstrom, October 18th, "US Population".
